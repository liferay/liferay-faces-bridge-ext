...
public abstract class LiferayURLGeneratorBaseImpl implements LiferayURLGenerator {
	...
	public static String encodeParameterName(String name, String encoding) throws UnsupportedEncodingException {
		...
		if (name != null) {
			...
			encodedName = encodedName.replace(" ", "+").replace("%20", "+");
		}
	...
	protected void appendParameterToURL(boolean firstParameter, boolean namespaced, String parameterName,
		...
		try {
			...
			parameterName = encodeParameterName(parameterName, encoding);
			...
			parameterValue = encodeParameterName(parameterValue, encoding);
